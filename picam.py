...
filepath = "/mnt/usb/webcam"
thumbpath = "/mnt/usb/thumbs"
...
saveWidth   = 1280
saveHeight  =1024
saveQuality = 50 # Set jpeg quality (0 to 100)

# settings of the photos to save
thumbWidth   = 362
thumbHeight  = 250
thumbQuality = 15 # Set jpeg quality (0 to 100)
...
def saveImage(settings, dt, width, height, quality, diskSpaceToReserve):
...
def saveThumbImage(settings, dt, width, height, quality, diskSpaceToReserve):
    time = dt
    filename = thumbpath + "/" + filenamePrefix + "-%04d%02d%02d-%02d%02d%02d.jpg" % (time.year, time.month, time.day, time.hour, time.minute, time.second)
    subprocess.call("raspistill %s -w %s -h %s -t 200 -e jpg -q %s -n -o %s" % (settings, width, height, quality, filename), shell=True)
    print "Captured %s" % filename
...
while (True):
    ...
    if takePicture:
        lastCapture = time.time()#
        dt = datetime.now()
        saveThumbImage(cameraSettings, dt, thumbWidth, thumbHeight, thumbQuality, diskSpaceToReserve)
        saveImage(cameraSettings, dt, saveWidth, saveHeight, saveQuality, diskSpaceToReserve)
